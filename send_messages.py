...
class Send_Message:
    ...
    def push_message(self,text):
        self.line_bot_api.push_message(extra_byte.group_id, messages=TextSendMessage(text=text))
...
class Get_time:
    ...
    def get_time(self,h, m):
        tme = timedelta(hours=0, minutes=0)
        if self.get_time_now() > timedelta(hours=h, minutes=m):
            return 'pass'
        while tme != timedelta(hours=h, minutes=m):
            tme = self.get_time_now()
            sleep(1)
        return 'not'
        # pass
